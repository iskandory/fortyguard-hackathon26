-- 0006 keyed 'unknown' off a MISSING exceedance row. But exceedance_etl
-- wrote a row of zeros whenever every month-chunk came back empty, so the
-- facility it knows nothing about (Equinix Ashburn DC4) had rows -- with
-- hours_exceeded = 0 -- and kept scoring 100/100 'safe'. The row exists; it
-- just says nothing.
--
-- exceedance_etl.py now records nothing when nothing was measured, which
-- fixes this at the source for future runs. This migration makes the view
-- correct against the rows already in the table, without waiting for a
-- re-run: a facility with zero exceedance hours, zero persistence AND no
-- observed reading has not been measured. A genuinely cool site would still
-- have a current reading, so it still classifies as 'safe'.
...
scored as (
  select
    ...
    ce.longest_run_hours as raw_run,
    lr.air_temp_c as raw_reading
  from facilities f
...
select
  ...
  case
    ...
    when raw_hours = 0 and coalesce(raw_run, 0) = 0 and raw_reading is null then 'unknown'
